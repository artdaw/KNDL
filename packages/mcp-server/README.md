...
| Tool | Description |
|------|-------------|
| `kndl_add_node` | Add a typed node with fields, confidence, source, validity, and decay |
...
| `kndl_add_edge` | Add a typed directed edge between two nodes |
...
| `kndl_parse` | Parse a KNDL document and merge it into the graph |
...
| `kndl_graph_stats` | Node/edge/intent counts and type distribution |
...
| `kndl://graph/summary` | Live node/edge/intent count summary |
...
```bash
uv sync --all-extras
uv run pytest tests/ -v   # 67 integration tests
```
